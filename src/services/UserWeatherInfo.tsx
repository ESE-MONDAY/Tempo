...
export const fetchWeatherInfo = async (userLocation: Coordinates): Promise<WeatherInfo> => {
  try {
    const cityResponse = await axios.get(`https://secure.geonames.org/findNearbyPlaceNameJSON?lat=${userLocation.latitude}&lng=${userLocation.longitude}&username=eselite`);
